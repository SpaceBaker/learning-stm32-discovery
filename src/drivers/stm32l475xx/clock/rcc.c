 ...
 * Date : 08 mai 2024
 * Author :  Émile Foricer
 *
 * Three internal clock sources
 * 	- High-speed internal 16MHz RC oscillator (HSI16)
 *	- Multi-speed internal RC oscillator (MSI)
 *	- Low-speed internal 32kHz RC oscillator (LSI)
 * Two external oscillators (needs external crystal/resonator)
 *	- High-speed external 4 to 48MHz oscillator (HSE) (with clock security system) 
 *	- Low-speed external 32.768kHz oscillator (LSE) (with clock security system)
...
void rcc_init(void)
{
	...
	SET_BIT(RCC->CR, (RCC_CR_MSIRGSEL | RCC_CR_MSIRANGE_7)); // 8MHz (default 4MHz)
